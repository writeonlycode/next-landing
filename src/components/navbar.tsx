...
import { siteMenu } from "@/lib/menu";

export default function Navbar({ menu }: { menu: siteMenu }) {
  const [show, setShow] = useState(false);

  const menuButton = (
    <button className="mx-2 w-8 h-8 relative z-50" onClick={() => setShow(!show)} aria-label="Toggle Menu">
      <span className={`${ show ? "opacity-0" : "" } transition absolute inset-0 flex justify-center items-center`} >
  ...
  const mobileContainer = (
    <div className={`${ show ? "h-screen" : "h-0" } w-screen overflow-hidden bg-isabelline transition-all duration-500 ease-in-out absolute inset-0 flex flex-col justify-center -mt-4`} >
      {menuItems}
    </div>
  );
